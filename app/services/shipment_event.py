...
class ShipmentEventService(BaseService):
    ...
    async def add(
        ...
        if not location or not status:
            ...
            if last_event is None:
                # no prior events -> use sensible defaults
                status = status or ShipmentStatus.placed
                # fallback to shipment.destination for location if available
                location = location or getattr(shipment, "destination", None)
            else:
    ...
    async def get_latest_event(self, shipment: Shipment):
        ...
        # timeline may be empty for new shipments
        if not timeline:
            return None
    ...
    async def _notify(self, shipment: Shipment, status: ShipmentStatus):
        ...
        match status:
            ...
            case ShipmentStatus.out_for_delivery:
                ...
                # persist verification code to Redis
                await add_shipment_verification_code(shipment.id, code)

                if shipment.client_contact_phone:
                    # interpolate the generated verification code into the message
                    send_sms.delay(
                        to=shipment.client_contact_phone,
                        body=f"Your order is arriving soon! Share the {code} code with your delivery executive to receive your package."
                    )
                else:
